...
def alpha_pass(transitions_matrix, emission_matrix, pi_matrix, emission_elements):
    ...
    alpha = [[0 for x in range(total_observations)]
    ...
    for state in range(total_states):
        alpha[state][0] = pi_matrix[0][state] * \
            emission_matrix[state][emission_elements[0]]
        ...
        for to_state in range(total_states):
            ...
            for from_state in range(total_states):
                ...
                alpha[to_state][observation] += alpha[from_state][observation -
                                                                  1] * transitions_matrix[from_state][to_state]
            # Multiply with the probability of observing o_t given that we are in state i
            alpha[to_state][observation] *= emission_matrix[to_state][emission_elements[observation]]
...
def beta_pass(transitions_matrix, emission_matrix, emission_elements, scale):
    ...
    # Create beta matrix
    # [ 1= [ALLA OBSERVATIONER], 2= [ALLA OBSERVATIONER] ] 1 och 2 är states
    beta = [[0 for x in range(total_observations)]
            for y in range(total_states)]
    ...
    for observation in range(total_observations-2, -1, -1):
        for from_state in range(total_states):
            ...
            for to_state in range(total_states):
                beta[from_state][observation] += transitions_matrix[from_state][to_state] * \
                    emission_matrix[to_state][emission_elements[observation+1]
...
def digamma(alpha, beta, transitions_matrix, emission_matrix, emission_elements):
    ...
    di_gamma = [[[0 for x in range(total_observation)] for y in range(
        total_states)] for z in range(total_states)]
    ...
    for observation in range(total_observation-1):
        for from_state in range(total_states):
            for to_state in range(total_states):
                ...
                di_gamma[to_state][from_state][observation] = alpha[from_state][observation] * transitions_matrix[from_state][to_state] * \
                    emission_matrix[to_state][emission_elements[observation+1]
                                              ] * beta[to_state][observation+1]
                ...
                # vi marginaliserar bort destinations variabeln från di-gamma kan man säga.
    for state in range(total_states):
...
def re_estimate_model_parameters(gamma, di_gamma, transitions_matrix, emission_matrix, pi_matrix, emission_elements):
    ...
    for state in range(total_states):  # för t = 0
    ...
    for from_state in range(total_states):
        ...
        for to_state in range(total_states):
            ...
            # sannoliketen att röra sig från state i till j
    ...
    for from_state in range(total_states):
        ...
        for emission in range(len(emission_matrix[0])):
            ...
            # sannolikhgeten att man ser en viss observation K givet state i.
...
def log_likelyhood(scale):
    ...
    log_prob = 0
    # P(alla observationer) = 1 /  summan av alla Cj
    for observation in range(total_observation):
        # vi tar fram sannolikheten att vi ser sekvens av observationer.
...
def learning_algorithm(transitions_matrix, emission_matrix, pi_matrix, emission_elements, max_iterations):
    ...
    alpha, scaling_factor = alpha_pass(
        transitions_matrix, emission_matrix, pi_matrix, emission_elements)

    for iteration in range(max_iterations):
        # Calculate the alpha, beta and gamma matrix
        alpha, scaling_factor = alpha_pass(
            transitions_matrix, emission_matrix, pi_matrix, emission_elements)
        beta = beta_pass(transitions_matrix, emission_matrix,
                         emission_elements, scaling_factor)
        gamma, di_gamma = digamma(
            alpha, beta, transitions_matrix, emission_matrix, emission_elements)

        # Re-estimate the model parameters
        transition_new, emission_new, pi_new = re_estimate_model_parameters(
            gamma, di_gamma, transitions_matrix, emission_matrix, pi_matrix, emission_elements)
...
def main():
    ...
    transition_output, emission_output = learning_algorithm(
        transitions_matrix, emission_matrix, pi_matrix, emission_elements, 100)
